    ...
    public function show( Request $request ){
        ...
        try {
          ...
          foreach ($response as $usuarios) {
              $data = [
                'id_rol'          => ($usuarios->roles )
                ,'id_empresa'     => ($usuarios->empresas )
                ,'id_sucursal'    => $usuarios->sucursales
                ,'name'           => $usuarios->name
                ,'first_surname'  => $usuarios->first_surname
                ,'second_surname' => $usuarios->second_surname
    ...
    public function store( Request $request){
          ...
          foreach ( $request->all() as $key => $value ) {
              if ( in_array($key, $claves_users) ) {
                  if ( $key == "email" ) { $request_users[$key] = strtolower($value);}
                  else{ $request_users[$key] = strtoupper($value); }
              }
